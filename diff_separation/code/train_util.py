...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.opt = th.optim.AdamW(self.master_params, lr=self.lr, weight_decay=self.weight_decay, betas=(0.9, 0.999))
        if self.resume_step:
    ...
    def run_loop(self):
        while (
            ...
            batch_vxz_p, batch_vxz, batch_mid_p, batch_vel, batch_loc, cond = next(self.data)

            self.run_step(batch_vxz_p, batch_vxz, batch_mid_p, batch_vel, batch_loc, cond)
    ...
    def run_step(self, batch_vxz_p, batch_vxz, batch_mid_p, batch_vel, batch_loc, cond):
        self.forward_backward(batch_vxz_p, batch_vxz, batch_mid_p, batch_vel, batch_loc, cond)
        if self.use_fp16:
            self.optimize_fp16()
        else:
