...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, vxz, vel, loc, t, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, vxz, vel, loc, self._scale_timesteps(t), **model_kwargs)
    ...
    def pde_loss(self, x, mid_p, dh):
        ...
        vxp_laplace = 1/(dh ** 2) * F.conv2d(x[:, 0:1], self.laplace_kernel, padding=self.fd_order//2, groups=1)
        vzp_laplace = 1/(dh ** 2) * F.conv2d(x[:, 1:2], self.laplace_kernel, padding=self.fd_order//2, groups=1)

        loss_vxp = mean_flat((vxp_laplace[:, :, self.fd_order//2:-self.fd_order//2,self.fd_order//2:-self.fd_order//2] - 
                              mid_p[:, 0:1, self.fd_order//2:-self.fd_order//2,self.fd_order//2:-self.fd_order//2]) ** 2)
        loss_vzp = mean_flat((vzp_laplace[:, :, self.fd_order//2:-self.fd_order//2,self.fd_order//2:-self.fd_order//2] - 
                              mid_p[:, 1:2, self.fd_order//2:-self.fd_order//2,self.fd_order//2:-self.fd_order//2]) ** 2)
        pde_loss = loss_vxp + loss_vzp
        # mean_flat((vxp_laplace[:, :, self.fd_order//2:-self.fd_order//2,self.fd_order//2:-self.fd_order//2] - mid_p[:, 0:1]) ** 2) + mean_flat((vzp_laplace - mid_p[:, 1:2]) ** 2)#th.sqrt(th.pow(loss_vxp,2) + th.pow(loss_vzp,2))
    ...
    def pde_guidance(self, x, mid_p, dh, weight_t, scale_factor=1.0):
        with th.enable_grad():
            ...
            pde_loss = mean_flat((vxp_laplace - mid_p[:, 0:1]) ** 2) + mean_flat((vzp_laplace - mid_p[:, 1:2]) ** 2)

            grad = th.autograd.grad(pde_loss, x, retain_graph=True)[0]
            grad_max = th.max(th.abs(grad)).item()
            grad = grad / grad_max

        return  2 * pde_loss * grad * scale_factor, th.sqrt((th.pow(loss_vxp,2)).mean() + (th.pow(loss_vzp,2)).mean()).item()


    def p_sample(
        self, model, x, vxz, mid_p, vel, loc, dh, t, scale_factor=1.0, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        pde_loss_after = 0.0
        if self.pde_guide:
            cond_grad, _ = self.pde_guidance(sample, mid_p, dh, out["variance"], scale_factor=scale_factor)
    ...
    def _vb_terms_bpd(
        self, model, x_start, x_t, vxz, vel, loc, t, clip_denoised=True, model_kwargs=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t, vxz, vel, loc, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
    ...
    def training_losses(self, model, x_start, vxz, mid_p, vel, loc, dh, t, model_kwargs=None, noise=None):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, vxz, vel, loc, self._scale_timesteps(t), **model_kwargs)
    ...
    def calc_bpd_loop(self, model, x_start, vxz, vel, loc, clip_denoised=True, model_kwargs=None):
        """
...
def laplace_operator(order=4, device='cuda'):
    if order == 2:
        ...
        laplace_kernel = th.tensor([[[[0, 0, 0, 0, -1/560, 0, 0, 0, 0],
                             [0, 0, 0, 0,  8/315,0, 0, 0, 0],
                             [0, 0, 0, 0, -1/5, 0, 0, 0, 0],
                             [0, 0, 0, 0, 8/5, 0, 0, 0, 0],
                             [-1/560, 8/315, -1/5, 8/5, -205*2/72,8/5,-1/5, 8/315,-1/560],
                             [0, 0, 0, 0, 8/5, 0, 0, 0, 0],
                             [0, 0, 0, 0, -1/5, 0, 0, 0, 0],
                             [0, 0, 0, 0, 8/315, 0, 0, 0, 0],
                             [0, 0, 0, 0, -1/560, 0, 0, 0, 0]]]], dtype=th.float32, device=device)
