...
# --------------------------
nshot = 5  # Number of shots per velocity model
nsnaps = 5  # Number of snapshots to save per shot

# --------------------------
# Load training velocity models
# --------------------------
vp_train = np.load('../dataset/velocity_model/vp_train.npy')  # Load P-wave velocity models
vs_train = np.load('../dataset/velocity_model/vs_train.npy')  # Load S-wave velocity models
...
for data_id in range(data_num):
    ...
    for ishot in range(nshot):
       ...
       if ishot == 0:
          nsz = 0
          nsx = random.randint(1, Nx - 2)
       else:
          nsz = random.randint(10, Nz - 10)
          nsx = random.randint(10, Nx - 10)
       ...
       # --------------------------
       random_it = generate_random_integers(100, Nt - 1, nsnaps)
       ...
       for it in range(Nt):
           ...
           if it in random_it:
               ...
               sio.savemat(f'{dir_train}data{data_id}_shot{ishot+1}_snap{snap_id}.mat', {
